...
class _SignupScreenState extends State<SignupScreen> with SingleTickerProviderStateMixin {
  ...
  void dispose() {
    ...
    _passwordCtrl.dispose(); _inviteCtrl.dispose();
    _focusName.dispose(); _focusEmail.dispose(); _focusPhone.dispose();
    _focusPassword.dispose(); _focusInvite.dispose();
    super.dispose();
  }
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: const Row(
          children: [
            ...
            SizedBox(width: 8),
            Text('Invitation Found', style: TextStyle(color: _textDark, fontWeight: FontWeight.bold)),
          ],
        ),
        content: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Container(
            ...
            child: Row(children: [
              const Icon(Icons.vpn_key_rounded, color: _accent, size: 20),
              const SizedBox(width: 8),
              Text('Code: $inviteCode', style: const TextStyle(color: _textDark, fontWeight: FontWeight.bold, fontSize: 16)),
            ]),
          ),
          const SizedBox(height: 12),
          Text('By joining, you will be able to see shared finances and collaborate.',
              style: TextStyle(color: Colors.black.withValues(alpha: 0.5), fontSize: 12, height: 1.4)),
        ]),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(ctx, false),
            child: const Text('Create New Household', style: TextStyle(color: _textMuted)),
          ),
          ElevatedButton(
    ...
    return Scaffold(
      body: Stack(
        children: [
          ...
          Positioned(
            top: -size.height * 0.1, right: -size.width * 0.18,
            child: Container(
          ...
          Positioned(
            top: size.height * 0.35, right: -size.width * 0.08,
            child: Container(
                  ...
                  child: Column(
                    children: [
                      ...
                      Container(
                        ...
                        child: ClipOval(child: Padding(padding: const EdgeInsets.all(4), child: Image.asset('assets/logo.png', fit: BoxFit.cover))),
                      ),
                      const SizedBox(height: 16),
                      const Text('Horizon', style: TextStyle(color: _textDark, fontSize: 28, fontWeight: FontWeight.w900, letterSpacing: -1)),
                      const SizedBox(height: 4),
                      const Text('Start your wealth journey', style: TextStyle(color: _textMuted, fontSize: 14, fontWeight: FontWeight.w500)),
                          ...
                          child: Container(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                const SizedBox(height: 14),
                                _glassField(controller: _phoneCtrl, focusNode: _focusPhone, icon: Icons.phone_outlined, hint: 'Phone (optional)', keyboardType: TextInputType.phone, nextFocus: _focusPassword),
                                const SizedBox(height: 14),
                                _glassField(controller: _passwordCtrl, focusNode: _focusPassword, icon: Icons.lock_outline_rounded, hint: 'Create a password', obscure: _obscurePassword, onToggleObscure: () => setState(() => _obscurePassword = !_obscurePassword), onSubmit: isLoading ? null : _handleSignup),
                                ...
                                SizedBox(
                                  ...
                                  child: ElevatedButton(
                                    ...
                                    child: isLoading
                                        ...
                                        : const Text('Create Account', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w800, letterSpacing: 0.3)),
                                  ),
                                ),
                              ],
                            ),
                      ...
                      Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                        const Text("Already have an account? ", style: TextStyle(color: _textMuted, fontSize: 14)),
                        GestureDetector(
                          onTap: () => Navigator.of(context).pop(),
                          child: const Text('Sign In', style: TextStyle(color: _accent, fontSize: 14, fontWeight: FontWeight.w700)),
                        ),
                      ]),
                      SizedBox(height: size.height * 0.05),
                    ],
  ...
  Widget _glassField({
    ...
    TextInputType? keyboardType, FocusNode? nextFocus,
    bool obscure = false, VoidCallback? onToggleObscure,
    VoidCallback? onSubmit, TextCapitalization textCapitalization = TextCapitalization.none,
  }) {
    return TextField(
      ...
      onSubmitted: (_) {
        ...
        else if (onSubmit != null) onSubmit();
      },
      ...
      decoration: InputDecoration(
        ...
        suffixIcon: onToggleObscure != null
            ? IconButton(
                icon: Icon(obscure ? Icons.visibility_outlined : Icons.visibility_off_outlined, color: _textHint, size: 20),
                onPressed: onToggleObscure,
              )
            : null,
        hintText: hint,
        hintStyle: const TextStyle(color: _textHint, fontWeight: FontWeight.w400, fontSize: 15),
        filled: true,
  ...
  Widget _passwordStrengthBar() {
    Color barColor;
    String label;
    if (_passwordStrength >= 0.8) { barColor = const Color(0xFF34D399); label = 'Strong'; }
    else if (_passwordStrength >= 0.5) { barColor = const Color(0xFFFBBF24); label = 'Medium'; }
    else if (_passwordStrength > 0) { barColor = const Color(0xFFF87171); label = 'Weak'; }
