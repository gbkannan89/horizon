package register

import (
	"context"
	"net/http"

	"github.com/jackc/pgx/v5/pgxpool"
	"github.com/horizon/core/services/experiences/accounts/internal/aggregator"
	"github.com/horizon/core/services/experiences/accounts/internal/api"
	"github.com/horizon/core/services/experiences/accounts/internal/engine"
	"github.com/horizon/core/services/experiences/accounts/internal/infrastructure/persistence"
)

func RegisterRoutes(mux *http.ServeMux, pool *pgxpool.Pool) {
	p := persistence.NewAccountsPGProvider(pool)
	agg := aggregator.New(aggregator.DataProviders{
		Accounts: p, Trans: p, Projection: p, Risk: p, Health: p,
		Recs: p, Optimize: p, Simulation: p, Events: p,
	}, genNoop{})
	h := api.New(agg, engine.NewComposer(), engine.NewEngine())
	h.Register(mux)
}

type genNoop struct{}
func (genNoop) Get(_ context.Context, _ string) (interface{}, bool) { return nil, false }
func (genNoop) Set(_ context.Context, _ string, _ interface{})      {}
func (genNoop) Invalidate(_ context.Context, _ string)              {}
