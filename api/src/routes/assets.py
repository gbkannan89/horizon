...
def add_asset(asset_in: AssetCreate, current_user: UserOut = Depends(get_current_user), db = Depends(get_db)):
    with db.cursor() as cur:
        try:
            cur.execute(
                """
                INSERT INTO assets (user_id, type, subtype, name, amount, interest_rate, start_date, maturity_date, is_liability, generates_income, income_frequency, purchase_price, purchase_date)
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                RETURNING id, user_id, type, subtype, name, amount, interest_rate, start_date, maturity_date, is_liability, generates_income, income_frequency, purchase_price, purchase_date, created_at
                """,
                (
                    current_user.id,
                    asset_in.type,
            ...
            db.commit()
            pp = float(row[12]) if row[12] is not None else None
            pd = row[13]
            return AssetOut(
                id=row[0],
                user_id=row[1],
...
ASSET_SELECT = "id, user_id, type, subtype, name, amount, interest_rate, start_date, maturity_date, is_liability, generates_income, income_frequency, purchase_price, purchase_date, created_at"
...
def list_assets(type: Optional[str] = None, current_user: UserOut = Depends(get_current_user), db = Depends(get_db)):
    with db.cursor() as cur:
        if type:
            cur.execute(
                f"SELECT {ASSET_SELECT} FROM assets WHERE user_id = %s AND type = %s",
                (current_user.id, type)
            )
        else:
            cur.execute(
                f"SELECT {ASSET_SELECT} FROM assets WHERE user_id = %s",
                (current_user.id,)
            )
        rows = cur.fetchall()
...
def _row_to_asset(r):
    return AssetOut(
        ...
        purchase_date=r[13], created_at=r[14]
    )
    ...
    with db.cursor() as cur:
        ...
        try:
            cur.execute(
                ...
                is_liability = %s, generates_income = %s, income_frequency = %s, purchase_price = %s, purchase_date = %s
                WHERE id = %s
                RETURNING {ASSET_SELECT}
                """,
                (
                    asset_in.type, asset_in.subtype, asset_in.name, asset_in.amount,
                    asset_in.interest_rate, asset_in.start_date, asset_in.maturity_date,
                    asset_in.is_liability, asset_in.generates_income, asset_in.income_frequency,
                    asset_in.purchase_price, asset_in.purchase_date, asset_id
                )
            )
            row = cur.fetchone()
            db.commit()
